...
//             alert("El control no ha sido enviado, hace falta ingresar el email del contacto");
...
// $dompdf = new DOMPDF();			
// $dompdf->load_html( $bodyhtml );
// $dompdf->render();
// $nombre_archivo = $request->getId() . ".pdf";
// $pdf = $dompdf->output();			
// file_put_contents("apps/veci/tmp/".$nombre_archivo, $pdf);

//Load Composer's autoloader
require '../../resources/lib/phpmailer/vendor/autoload.php';
...
try {
    ...
    $mail->Host       = 'mail.nobutic.com';                   //Configurar el servidor para enviar a través SMTP
    $mail->SMTPAuth   = true;                                 //Habilitar autenticación SMTP
    $mail->Username   = 'soporte@insuempresa.com';      //SMTP username
    $mail->Password   = '50p0rt3cl0ud';                   //SMTP password
    ...
    //Recipients
    $mail->setFrom('soporte@insuempresa.com', 'Soporte INSU TI');  //Add a recipient  
    $mail->addAddress($email, $cliente);                      //($email, $NombreUsuario)
    // $mail->addReplyTo('info@example.com', 'Information');
    // $mail->addCC('cc@example.com');
    // $mail->addBCC('bcc@example.com');

    //Archivos adjuntos
    $mail->addAttachment("apps/veci/tmp/".$filename);         // ($ubicacion_control)
    // $mail->addAttachment('/var/tmp/file.tar.gz');          //Agregar archivos adjuntos
    // $mail->addAttachment('/tmp/image.jpg', 'new.jpg');     //Optional name
    
    //Content
    $mail->isHTML(true);                                      //Establecer el formato de correo electrónico en HTML
    $mail->Subject = 'CONTROL DE SERVICIO '.$cliente;
    $mail->Body    = 'Buen día,
                    <br/><br/>
                    Adjunto encontrará el control de servicio prestado por el asesor el día de hoy. En caso de Presentar alguna inconformidad con el detalle descrito, es necesario comunicarlo respondiendo el correo que entrega el presente. Luego de tres (3) días hábiles de enviado este soporte, si no existe notificación alguna se dará la aceptación y se precederá a descontar el tiempo de servicio prestado.
                    <br/><br/>
